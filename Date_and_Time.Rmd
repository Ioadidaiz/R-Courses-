...
receivables <- receivables %>%
  mutate(days_to_settle = settled_date - invoice_date)# pipe operator creates new column with "Settled Date - Invoice Date"
...
receivables_3 <- receivables %>%
  ...
  dplyr::filter(delay < 0) %>% # filter for actually delays
  dplyr::mutate(
    delay = delay * -1        # change to positive number
  ) %>% 
  dplyr::count(delay)       # counts how often a certain delay happened -> 1 day delay = 61 times (n) ...
